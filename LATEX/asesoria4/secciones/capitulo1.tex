...
%  Aportes numerados A1..A9, secuenciales en orden de lectura.
...
\textbf{nueve aportes} que el proyecto declara, tres por cada objetivo
específico. El número de aportes no coincide con el de publicaciones porque un
mismo estudio puede dar origen a más de uno, según cuántos de sus resultados se
estén explotando: de HyperNCD nacen dos, de BRDL tres, de Superpoint Transformer
dos, y de Sonata y de PTv3 uno cada uno.
...
Sección~\ref{sec:hyperncd}, el aporte A1: aporta la pieza que se coloca en
...
\begin{enumerate}
\item[\textbf{A8}] \textbf{Medición del canal geométrico en la combinación de
...
\item[\textbf{A9}] \textbf{Reporte desagregado del costo, que la literatura de
...
Sobre esa base el proyecto declara \textbf{nueve aportes, tres por cada objetivo
específico}. Cada uno se enunció ya en la sección de la que nace, y la
Tabla~\ref{tab:obj-vs-revision} los reúne.
...
parámetros que gobiernan la afinidad entre prototipos (A6) y verificar que la
...
operación.} Tres aportes: Formular una métrica que separe la contribución de la
representación de la del razonamiento (A3), medir el valor del canal geométrico
en la única combinación de dominio y régimen que ningún trabajo ha reportado (A8)
y reportar el costo de forma desagregada sobre hardware verificado (A9).

\begin{table}[htbp]
\centering
\small
\caption{Inventario de los nueve aportes del proyecto, tres por cada objetivo
específico. Cada uno nace de un estudio revisado y termina en un resultado propio
que se puede verificar. La numeración sigue el orden de lectura del capítulo.}
