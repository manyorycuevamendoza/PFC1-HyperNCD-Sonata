...
\begin{table}[H]
\centering
\caption{Especificación del hardware verificado en el clúster Khipu (UTEC).}
\label{tab:hardware}
\begin{tabular}{@{}p{3.6cm}p{4.2cm}p{4.2cm}@{}}
\toprule
\textbf{Recurso} & \textbf{Especificación} & \textbf{Uso en el proyecto} \\
\midrule
Nodo de cómputo & \textit{[completar: modelo]} & Ejecución de entrenamiento \\
GPU & \textit{[completar: modelo y cantidad]} & Rama semántica y codificador \\
VRAM por GPU & \textit{[completar]}\,GB & Ventana por paso \\
RAM del nodo & \textit{[completar]}\,GB & Carga de sección voxelizada \\
Almacenamiento & \textit{[completar]} & Nube voxelizada y \emph{cache} \\
Gestor de colas & Slurm & Envío de corridas \\
\bottomrule
\end{tabular}
\end{table}
