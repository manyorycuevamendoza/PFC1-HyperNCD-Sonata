...
\node[mod, draw=cSem, fill=cSem!5, text width=3.1cm] (M2) at (5.6,16.2)
 {\textcolor{cSem}{\textbf{Módulo 2. ¿A qué se parece cada punto?}}\\[1pt]
  {\tiny\itshape\color{cSem!85}rama semántica de HyperNCD}\\[1pt]
  {\tiny\color{black!55}depende de: Módulo 1}\\[3pt]
...
\node[mod, draw=cEnc, fill=cEnc!5, text width=3.1cm] (M3) at ($(M2.south)+(0,-0.55)$)
 {\textcolor{cEnc}{\textbf{Módulo 3. ¿Qué forma hay alrededor?}}\\[1pt]
  {\tiny\itshape\color{cEnc!85}codificador de Sonata}\\[1pt]
  {\tiny\color{black!55}depende de: Módulo 1}\\[3pt]
  \tikz[baseline=0, every node/.style={anchor=center}]{
...
\node[mod, draw=cEnt, fill=cEnt!5, text width=2.3cm, anchor=east] (M1) at ($(eje)-(4.85,0)$)
 {\textcolor{cEnt}{\textbf{Módulo 1. Lo que entra}}\\[1pt]
  {\tiny\itshape\color{cEnt!85}datos del proyecto}\\[1pt]
  {\tiny\color{black!55}depende de: --}\\[3pt]
...
\node[mod, draw=cFus, very thick, fill=cFus!4, text width=2.6cm, anchor=west] (M4) at ($(eje)+(0.9,0)$)
 {\textcolor{cFus}{\textbf{Módulo 4. Se juntan y se agrupa}}\\[1pt]
  {\tiny\itshape\color{cFus!85}razonamiento de HyperNCD}\\[1pt]
  {\tiny\color{black!55}depende de: Módulos 2 y 3}\\[3pt]
  \tikz[baseline=0, every node/.style={anchor=center}]{
...
\node[mod, draw=cSal, fill=cSal!6, text width=2.2cm, anchor=west] (M5) at ($(M4.east)+(1.5,0)$)
 {\textcolor{cSal}{\textbf{Módulo 5. Lo que sale}}\\[1pt]
  {\tiny\itshape\color{cSal!85}resultado del sistema}\\[1pt]
  {\tiny\color{black!55}depende de: Módulo 4}\\[3pt]
...
\node[tarj] (T1) at ($(pc)+(-5.45,-1.15)$)
 {\textbf{\textcircled{\scriptsize 1} Se juntan las dos descripciones}\\[4pt]
  ...
  \raggedright
  $F_i$ es la descripción completa del punto $i$: las dos anteriores guardadas
  juntas, sin mezclarse ni promediarse. Pasa de 102 a 1184 valores, porque los
  6 números de forma hechos a mano se reemplazan por los 1088 aprendidos.
  \textbf{Ese reemplazo es el aporte}.};
...
\node[tarj] (T2) at ($(pc)+(0,-1.15)$)
 {\textbf{\textcircled{\scriptsize 2} Cada punto se reparte entre los grupos}\\[4pt]
  ...
  \raggedright
  $s_{i,k}$ mide cuánto pertenece el punto $i$ al grupo $k$, y lo que reparte
  entre todos los grupos suma 1. Al no obligarlo a elegir uno solo, un punto en
  el borde entre dos piezas puede pertenecer en parte a cada una. Con esos
  repartos se calcula el representante de cada grupo.};
...
\node[tarj] (T3) at ($(pc)+(5.45,-1.15)$)
 {\textbf{\textcircled{\scriptsize 3} Los grupos se relacionan entre sí}\\[4pt]
  ...
  $\alpha$ decide cuánto pesa cada uno: con $\alpha=1$ solo la forma, con
  $\alpha=0$ solo el significado. Cada grupo se enlaza con los $M=8$ más
  parecidos, y como el enlace une varios a la vez, una pieza nunca vista queda
  descrita como combinación de varias conocidas.};
